...
class SimpleEmbedding(nn.Module):
    """
    A simple linear/non-linear transformation
    """

    def __init__(self, input_dim, d_model, drop_prob):
        """
        class for word embedding that included positional information
        :param vocab_size: size of vocabulary
        :param d_model: dimensions of model
        ...
        self.embedding = nn.Sequential(
            nn.Linear(input_dim, d_model), nn.Tanh())
        self.drop_out = nn.Dropout(p=drop_prob)
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim, d_model, ffn_hidden, n_head, n_layers, drop_prob):
        super().__init__()
        self.emb = SimpleEmbedding(input_dim=input_dim,
                                   d_model=d_model,
                                   drop_prob=drop_prob,
                                   )
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, n_layers, d_model, n_head, ffn_hidden, drop_prob, input_dim, src_pad_idx):
        super().__init__()
    ...
    def forward(self, inputs_embeds, attention_mask):
        ...
        # attention_mask = torch.sum(src, dim=-1)
        src_mask = self.make_pad_mask(attention_mask, attention_mask)
        output = self.encoder(inputs_embeds, src_mask)
        return output
...
class Transformer(nn.Module):
    ...
    def make_no_peak_mask(self, q, k):
        ...
        # len_q x len_k
        mask = torch.tril(torch.ones(len_q, len_k)).type(
